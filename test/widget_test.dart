...
void main() {
  group('Widget', () {
    testWidgets('Pump RePaint', (tester) async {
      ...
      expect(
        ...
        allOf(
          ...
          isA<Element>(),
          isA<RePaintElement>()
              .having((e) => e.widget, 'widget', same(widget))
              .having(
                (e) => e.renderObject,
                'renderObject',
                allOf(isNotNull, isA<RenderObject>(), isA<RePaintBox>()),
              ),
        ),
      );
      ...
      expect(
        ...
        allOf(
          ...
          isA<RePaintBox>()
              ...
              .having((r) => r.isRepaintBoundary, 'isRepaintBoundary', true)
              .having((r) => r.painter, 'painter', same(widget.painter))
              .having((r) => r.attached, 'attached', true)
              .having((r) => r.owner, 'owner', isNotNull)
              .having((r) => r.hasSize, 'hasSize', isTrue)
              .having(
                (r) => r.size,
                'size',
                allOf(isNotNull, isA<Size>(), isNot(equals(Size.zero))),
              ),
        ),
      );
    ...
    testWidgets('Implicit', (tester) async {
      await tester.pumpWidget(
        RePaint.inline(
          ...
          update: (box, paint, _) =>
              paint
                ..color = paint.color == Colors.red ? Colors.blue : Colors.red,
    ...
    testWidgets('Implicit empty', (tester) async {
      await tester.pumpWidget(RePaint.inline(render: (_, __, ___) {}));
      expect(find.byType(RePaint), findsOneWidget);
      await tester.pump(const Duration(milliseconds: 32));
      expect(find.byType(RePaint), findsOneWidget);
      await tester.pumpWidget(const SizedBox.shrink());
