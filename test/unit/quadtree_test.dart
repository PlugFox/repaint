...
void main() => group('Quadtree', () {
  ...
  test('Query', () {
    ...
    expect(
      ...
      isA<QuadTree$QueryResult>()
          .having((qr) => qr.isEmpty, 'isEmpty', isFalse)
          .having((qr) => qr.isNotEmpty, 'isNotEmpty', isTrue)
          .having((qr) => qr.length, 'length', 1),
    );
    final map = result.toMap();
    expect(map, allOf(isA<Map<int, ui.Rect>>(), hasLength(1)));
  });
  ...
  test('Create/Move/Query/Remove/Optimize/Clear', () {
    ...
    expect(qt.get(10), allOf(isNotNull, isA<ui.Rect>()));
    for (var i = 0; i < 10; i++) {
      expect(() => qt.move(10, i * 10.0, i * 10.0), returnsNormally);
    }
    expect(qt.length, equals(10));
    expect(
